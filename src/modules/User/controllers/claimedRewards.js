...
export const claimedRewardsController = async (req, res) => {
  const { wallet,keyID, rewardID, rewardAmount } = req.body;
  console.log(wallet,keyID, rewardID, rewardAmount)
  try {
    ...
    return successResponse({ res, response: { user: existingUser, totalKeyInfo: totalKeyInfo } });
